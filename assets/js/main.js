...
// Agregando horas al arreglo de Traumatología
traumatologia.push (
...
// tabla para extraer la información de cada objeto y unirla en una cadena de texto

información = dental.map(datos => `${datos.hora} - ${datos.especialista} - ${datos.paciente} - ${datos.rut} - ${datos.prevision}`);
tabla = información.join(" - ");
...
let consolidado = traumatologia.concat(radiologia, dental).map(consulta => ` ${  consulta.paciente}`);
console.log(consolidado);

// 5. Filtrar aquellos pacientes que indican ser de ISAPRE en la lista de consultas médicas de Dental

let pacientesIsapre = dental.filter(consulta => consulta.prevision === 'ISAPRE').map(consulta => ` ${  consulta.paciente} - ${consulta.prevision}`);
...
let pacientesFonasa = traumatologia.filter(consulta => consulta.prevision === 'FONASA').map(consulta => ` ${  consulta.paciente} - ${consulta.prevision}`);
